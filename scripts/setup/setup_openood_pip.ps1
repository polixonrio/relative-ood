...
function Resolve-BootstrapPython {
    ...
    if ($RequestedVersion) {
        if ($null -eq $py) {
            throw "PythonVersion '$RequestedVersion' was requested, but the Windows py launcher was not found on PATH."
        }

        $versionFlag = if ($RequestedVersion.StartsWith('-')) {
            $RequestedVersion
        }
        else {
            "-$RequestedVersion"
    ...
    if ($null -ne $py) {
        $launcherListing = @()
        try {
            $launcherListing = & $py.Source -0p 2>$null
        }
        catch {
            $launcherListing = @()
        }
        ...
        foreach ($preferredVersion in @('3.12', '3.11')) {
            $matchingLine = $launcherListing |
                Where-Object { $_ -match [regex]::Escape($preferredVersion) } |
                Select-Object -First 1
            if ($matchingLine -and $matchingLine -match '(?<path>[A-Za-z]:\\.*python\.exe)$') {
                return @{
    ...
    if ($null -eq $python) {
        throw 'Python was not found on PATH. Install Python or use the uv setup script instead.'
    }
...
$pipExe = Join-Path $venvAbsolutePath 'Scripts\pip.exe'
...
if (-not (Test-Path $venvAbsolutePath)) {
    Write-Step 'Creating virtual environment with stdlib venv'
    $bootstrap = Resolve-BootstrapPython -RequestedVersion $PythonVersion
    $venvArgs = @($bootstrap.Arguments + @($venvAbsolutePath))
    Invoke-External -FilePath $bootstrap.FilePath -Arguments $venvArgs -WorkingDirectory $repoPath
}
else {
...
if (-not (Test-Path $pipExe)) {
    throw "Expected virtual environment pip not found: $pipExe"
}
...
if (-not $SkipInstall) {
    ...
    if ($shouldInstall) {
    Write-Step 'Installing repo and required analysis dependencies with pip'
    Write-Host "Using PyTorch backend $($torchProfile.Backend) ($($torchProfile.Description))." -ForegroundColor DarkGray

    Invoke-External -FilePath $pythonExe `
        -Arguments @('-m', 'pip', 'install', '--upgrade', 'pip', 'wheel', 'setuptools<82') `
        -WorkingDirectory $repoPath

    Invoke-External -FilePath $pipExe `
        -Arguments @('install', 'numpy<2') `
        -WorkingDirectory $repoPath

    Invoke-External -FilePath $pipExe `
        -Arguments @('install', '--upgrade', '--force-reinstall', '--index-url', $torchProfile.PipIndexUrl, 'torch', 'torchvision') `
        -WorkingDirectory $repoPath

    Invoke-External -FilePath $pipExe `
        -Arguments @('install', '-e', $repoPath) `
        -WorkingDirectory $repoPath
    ...
    Invoke-External -FilePath $pipExe `
        -Arguments @('install', 'statsmodels', 'timm', 'pyarrow') `
        -WorkingDirectory $repoPath

    if (-not $SkipLibMR) {
        Invoke-External -FilePath $pipExe `
            -Arguments @('install', '--no-build-isolation', 'libmr') `
            -WorkingDirectory $repoPath
    }
